...
function renderCurrentQuestion() {
  ...
  quizQuestionText.innerHTML = `${imageHtml}${q.testo_domanda}`;
  ...
  quizAnswersArea.innerHTML = shuffledAnswers.map((a, i) => `
    <button class="answer-card w-full p-4 sm:p-5 flex flex-col items-start gap-3 text-left group" data-index="${i}">
      <div class="flex items-center gap-4 w-full">
        ...
        <span class="flex-1 font-semibold text-slate-700 dark:text-slate-200 text-base sm:text-lg">${a.testo_risposta}</span>
      </div>
      <div class="explanation-area hidden w-full pl-12 text-sm text-slate-600 dark:text-slate-400 opacity-90">
